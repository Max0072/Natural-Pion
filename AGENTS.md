...
| can one rotation side be dropped | **no** — measured per side with independent step sizes, the contributions are additive to within 5% in all nine layers tested, so dropping either costs its whole share. `out` carries 0.49-0.77 and `in` 0.25-0.54, with `in` rising with depth. This also explains why their `alternate` costs only 0.23%: independent sides serialise cheaply |
