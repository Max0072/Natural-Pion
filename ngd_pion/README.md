...
|---|---|---|---|
| `ngd-pion-s` | `FastNGDPionS` | `with_s_fast.py` | **live.** Every NGD result in the repository. Bit-identical to its reference; the "fast" in the name is a leftover, see its docstring |
| `ngd-pion-m` | `MomentumNGDPionS` | `momentum.py` | **live, best number so far.** `ngd-pion-s` plus a first moment. 3.8734 at 3000 steps against `pion`'s best-of-eight 3.9880 |
...
| `ngd-pion` | `FastNGDPion` | `fast.py` | superseded. `S = I`. The one full-length NGD run (3.6728) used it; the measured `S` beats it by 0.23-0.32 |
